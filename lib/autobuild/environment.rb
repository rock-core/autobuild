...
module Autobuild
    ...
    SHELL_HELPER_SCRIPT = SHELL_SOURCE_SCRIPT
    SHELL_HOOK_SCRIPT =
        if windows? then '"%s" "$@"'.freeze
        else
            '. "%s" "$@"'.freeze
    ...
    class Environment
        ...
        #   @param [String] declare a path that should be added to source_before
        #
        def source_before(file = nil, shell: 'sh', type: 'helper')
            if file
                script_spec = { file: file, shell: shell }

                if type then script_spec[:type] = type end

                @source_before << script_spec

                source_before(script_spec.reject { |k, v| k == :file }) # for backwards compatibility
            else
                @source_before.select { |item| item[:shell] == shell && item[:type] == type }
                               .map { |item| item[:file] }
            end
        end
        ...
        #   @param [String] declare a path that should be added to source_after
        #
        def source_after(file = nil, shell: 'sh', type: 'helper')
            if file
                script_spec = { file: file, shell: shell }

                if type then script_spec[:type] = type end

                @source_after << script_spec

                source_after(script_spec.reject { |k, v| k == :file }) # for backwards compatibility
            else
                @source_after.select { |item| item[:shell] == shell && item[:type] == type }
                              .map { |item| item[:file] }
            end
        end
        ...
        def export_env_sh(io, shell: 'sh', type: 'helper')
            export = exported_environment

            source_before(shell: shell, type: 'helper').each do |path|
                io.puts format(SHELL_HELPER_SCRIPT, path)
            end

            source_before(shell: shell, type: 'hook').each do |path|
                io.puts format(SHELL_HOOK_SCRIPT, path)
            end
            ...
            source_after(shell: shell, type: 'helper').each do |path|
                io.puts format(SHELL_HELPER_SCRIPT, path)
            end

            source_after(shell: shell, type: 'hook').each do |path|
                io.puts format(SHELL_HOOK_SCRIPT, path)
            end
        end
