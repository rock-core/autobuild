...
module Autobuild
	...
	# Returns the module to get
		...
		if !File.exists?("#{package.importdir}/CVS/Root")
		    raise ConfigException.new(package, 'import'), "#{package.importdir} is not a CVS working copy"
		end

		root = File.open("#{package.importdir}/CVS/Root") { |io| io.read }.chomp
		mod  = File.open("#{package.importdir}/CVS/Repository") { |io| io.read }.chomp
		...
		if root != expected_root || mod != @module
		    raise ConfigException.new(package, 'import'), "checkout in #{package.importdir} is from #{root}:#{mod}, was expecting #{expected_root}:#{@module}"
		end
        ...
        def checkout(package) # :nodoc:
            head, tail = File.split(package.importdir)
            cvsroot = @root
